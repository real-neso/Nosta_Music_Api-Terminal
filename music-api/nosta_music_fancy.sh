...
# ============================================
# 🎵 NOSTA MUSIC MANAGER - Terminal Interface v3.6
# ============================================
# Author: Nosta CLI Manager
# Version: 3.6.0 - Fixed Artist Update (PUT with proper JSON)
# Terminal: Kitty (graphics support)
# Player: mpv
# ============================================
...
print_header() {
    ...
    echo "║                                                              ║"
    echo "║     Terminal Interface v3.6.0 | Kitty + mpv                ║"
    echo "║                                                              ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
print_menu() {
    ...
    echo -e "${YELLOW}${BOLD}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  1)${NC} 🎵 List All Songs"
    echo -e "${GREEN}  2)${NC} 🔍 Search Songs"
    echo -e "${GREEN}  3)${NC} 📤 Upload Song(s)"
    echo -e "${GREEN}  4)${NC} ✏️  Edit Song"
    echo -e "${GREEN}  5)${NC} 🗑️  Delete Song"
    echo -e "${GREEN}  6)${NC} 🗑️  Delete Multiple Songs"
    echo -e "${GREEN}  7)${NC} ⭐ View Favorites"
    echo -e "${GREEN}  8)${NC} 👤 User Profile"
    echo -e "${GREEN}  9)${NC} 📊 Statistics"
    echo -e "${GREEN}  10)${NC} ▶️  Now Playing"
    echo -e "${GREEN}  11)${NC} 🔄 Update Artists (from server)"
    echo -e "${GREEN}  12)${NC} 📁 Update Artists (from local folder)"
    echo -e "${GREEN}  0)${NC} 🚪 Logout & Exit"
    echo -e "${YELLOW}${BOLD}═══════════════════════════════════════════════════════════════${NC}"
...
# ============================================
# 🖼️ KITTY GRAPHICS - REAL IMAGE DISPLAY
# ============================================
display_image_kitty() {
    local url="$1"
...
# ============================================
# 👤 USER PROFILE WITH KITTY AVATAR
# ============================================
display_user_info() {
    local user=$(get_user_info)
    ...
    echo -e "\n${CYAN}${BOLD}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}${BOLD}  👤 USER PROFILE${NC}"
    echo -e "${CYAN}${BOLD}═══════════════════════════════════════════════════════════════${NC}"
    ...
    if [ "$role" = "VIP" ]; then
        ...
        echo -e "${WHITE}${BOLD}⭐ Role:${NC}     ${BLUE}Normal${NC}"
    fi
...
play_song() {
    ...
    print_success "▶️  Now Playing: $title - $artist"
}
...
stop_song() {
    if [ -n "$MPV_PID" ] && kill -0 "$MPV_PID" 2>/dev/null; then
        ...
        CURRENT_SONG=""
        CURRENT_SONG_ID=""
        print_info "⏹️  Stopped"
    fi
}

show_now_playing() {
    if [ -n "$CURRENT_SONG" ]; then
        echo -e "\n${GREEN}${BOLD}▶️  NOW PLAYING${NC}"
...
# ============================================
# 📤 UPLOAD FUNCTIONS (TELEGRAM STORAGE)
# ============================================
upload_single_file() {
    ...
    if [ -f "$cover_file" ] && [ -s "$cover_file" ]; then
        ...
        echo -e "${YELLOW}⚠️  No cover art found${NC}"
        cover_file=""
    fi
    ...
    if echo "$response" | jq -e '.id' >/dev/null 2>&1; then
        ...
        echo -e "${BLUE}⏱️  Time: ${WHITE}${duration}s${NC}  ${BLUE}📊 Speed: ${WHITE}${speed} MB/s${NC}"
...
upload_folder() {
    ...
    for ((i=0; i<total_files; i++)); do
        ...
        if echo "$response" | jq -e '.id' >/dev/null 2>&1; then
            ...
            echo -e "${DIM}   ⏱️  ${file_duration}s | 📊 ${speed} MB/s | 🆔 ${song_id:0:8}${NC}"
        else
    ...
    echo -e "${BLUE}⏱️  Total time: ${WHITE}${total_duration}s${NC}"
...
# ============================================
# 🎵 SONG LISTING WITH PLAYER SELECTION (OPTIMIZED)
# ============================================
list_songs() {
...
# ============================================
# 🔄 UPDATE ARTISTS - استخراج من الخادم
# ============================================
update_artists() {
    ...
    while IFS= read -r song; do
        ...
        if [ "$new_artist" != "$current_artist" ] && [ "$new_artist" != "Unknown Artist" ]; then
            ...
            skipped=$((skipped + 1))
            echo -e "   ${DIM}⏭️  No change needed${NC}"
        fi
    ...
    echo -e "${DIM}⏭️  Skipped: $skipped${NC}"
...
# ============================================
# 📁 UPDATE ARTISTS FROM LOCAL FOLDER
# ============================================
...
# ============================================
# ✏️ EDIT SONG (REPLACE WITH NEW FILE)
# ============================================
edit_song() {
    print_subheader "✏️ EDIT SONG"
    ...
    case $edit_choice in
        1)
            ...
            if echo "$upload_response" | jq -e '.id' >/dev/null 2>&1; then
                ...
                echo -e "${YELLOW}⚠️  Note: The ID has changed. The old ID is no longer valid.${NC}"
            else
...
delete_song() {
    ...
    echo -e "${RED}⚠️  Delete song: ${BOLD}$song_id${NC}${RED}?${NC}"
...
delete_multiple_songs() {
    print_subheader "🗑️ DELETE MULTIPLE SONGS"
    ...
    if [ "$song_ids_input" = "all" ]; then
        echo -e "${RED}${BOLD}⚠️  DELETE ALL SONGS!${NC}"
    ...
    echo -e "${RED}⚠️  Delete ${total} song(s)?${NC}"
...
main_menu() {
    while true; do
        ...
        if [ -f "$USER_FILE" ] && [ -s "$USER_FILE" ]; then
            ...
            if [ -n "$CURRENT_SONG" ]; then
                echo -e "${MAGENTA}▶️  $CURRENT_SONG${NC}"
            fi
        else
